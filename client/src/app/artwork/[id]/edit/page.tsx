...
interface PopulatedArtwork extends Omit<Artwork, 'images'> {
...
export default function EditArtworkPage() {
  ...
  const { data: artwork, isLoading } = useQuery<PopulatedArtwork>({
  ...
  // Populate form when artwork data is loaded
  useEffect(() => {
    if (artwork) {
      setTitle(artwork.title || '')
      setDescription(artwork.description || '')
      setTags(artwork.tags || [])
      setIsPublished(artwork.isPublished ?? true)
      setExistingImages(artwork.images?.map((img: PopulatedImage) => img.url) || [])
      setImagePreviews(artwork.images?.map((img: PopulatedImage) => img.url) || [])
    }
  }, [artwork])
  ...
  const removeImage = (index: number) => {
    ...
    if (index < existingImages.length) {
      ...
      // If it's a newly uploaded image, remove from images array
    ...
    <div className="min-h-screen bg-black">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-8">
          ...
          <p className="text-zinc-400">Update your artwork details</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="border-2 border-dashed border-zinc-700 rounded-lg p-8 text-center">
                  ...
                  <p className="text-zinc-400 mb-4">
                    Add more images to your artwork
                  </p>
                  <Input
                    type="file"
                    multiple
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button
              type="submit"
              disabled={uploading || !title.trim() || updateArtworkMutation.isPending}
              className="bg-zinc-800 hover:bg-zinc-700 text-white"
            >
              {uploading || updateArtworkMutation.isPending ? 'Updating...' : 'Update Artwork'}
            </Button>
          </div>
        </form>
      </div>
